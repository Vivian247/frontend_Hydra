...
backendUrl = "http://localhost:3000/signin";
  ...
  */
  const updateUserHandler = async (data, userID, token) => {
    try {
      ...
      const res = await fetch(`${backendUrl}/User/edit/${userID}`, {
        method: "PUT",
        headers: {
  ...
  /*
 This function is used to fetch a user's details by their ID.
*/
const getUserById = async (userId, token) => {
    try {
      uiContext.setLoading(true);
      const res = await fetch(`${backendUrl}/User/get/${userId}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
      ...
      if (!res.ok) {
        const resData = await res.json();
        const errorMessage = resData.message || "Unable to fetch user details, try again.";
        throw new Error(errorMessage);
      }
      const resData = await res.json();
      uiContext.setLoading(false);
      setUserData(resData.data); // Update your state with the fetched user data
      uiContext.setSnackBar({
        show: true,
        success: true,
  ...
  // Then, include getUserById in your contextValue object as shown previously
const contextValue = {
    userData: userData,
    setUserData,
